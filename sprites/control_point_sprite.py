...
class ControlPointSprite(pygame.sprite.Sprite):
    ...
    def update(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.selected = self.rect.collidepoint(mouse_pos) and any(mouse_buttons)
        self.image = self.selected_image if self.selected else self.original_image

        if self.selected:
            self.rect = self.image.get_rect(center=(mouse_pos[0], mouse_pos[1]))
            ...
            self.x: float = (
                self.control_points.get_quartet(self.q_index).get_point(self.p_index).x
            )
            self.y: float = (
                self.control_points.get_quartet(self.q_index).get_point(self.p_index).y
            )
